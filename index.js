			...
			if (isLimit(sender, isPremium, isCreator, isOwner, limitawal, limit)) return sendButMessage(from, lang.limit(prefix), `© ${ownername}`, [{buttonId: 'limit', buttonText: {displayText: `Check Limit`, },type: 1,}]);
					...
					if (isLimit(sender, isPremium, isCreator, isOwner, limitawal, limit)) return sendButMessage(from, lang.limit(prefix), `© ${ownername}`, [{buttonId: 'limit', buttonText: {displayText: `Check Limit`, },type: 1,}]);
                        ...
                        await ffmpeg(`./${media}`)
                           ...
                           .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(260,iw)':min'(260,ih)':force_original_aspect_ratio=decrease,fps=60, pad=260:260:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
                            .toFormat('webp')
                    ...
                    //    ran = getRandom('.webp')
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', async function () {
								...
								.addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(190,iw)':min'(190,ih)':force_original_aspect_ratio=decrease,fps=60, pad=190:190:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
                            .toFormat('webp')
                            .save(`./sticker/${sender}.webp`)
